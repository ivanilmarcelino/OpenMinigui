...
FUNCTION MenuAchoice(aMenu, aParWin, aParMenu, aParIco, aIcoWin, lTxtAlign)
   ...
   DEFAULT aParWin  := { "Form_Achoice", "No menu name", "not.ico", MYBACK, 0, 0, 0, 0 }
   ...
   IF LEN(aParIco) == 0
      ...
      aBtnBClr  := { GRAY , BLACK  }                // цвет фона кнопок
   ...
   nHWin   := CalcWinHeight(nG,aMenu,nHBtn) + nG
   nWVirt  := NIL
   nHVirt  := NIL
   ...
   ELSE
      nH := nHWin - GetBorderHeight()   // установим внешнюю высоту окна
   ENDIF
