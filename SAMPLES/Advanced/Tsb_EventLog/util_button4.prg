...
FUNCTION my2BUTTON(y, x, w, h, cObj, cCapt, aBtnGrad, aBtnClr, aIcon, aFntClr, aFnt, nwPost, lBlock, lHide, i )
   LOCAL aGrOver, aGrFill, nSizeIcon, lSizeIcon, y1, x1, lTextVertical, nWIco, nWCap, o
   ...
   ELSE
      ...
      ELSE
         IF lTextVertical  //  VERTICAL
            @ y, x  BUTTONEX &cObj  PARENT &cForm                                       ;
              WIDTH w HEIGHT h CAPTION cCapt ICON aIcon[1]                              ;
              NOXPSTYLE HANDCURSOR NOTABSTOP NOHOTLIGHT FLAT                            ;
              FONTCOLOR aFntClr[1] BACKCOLOR aBtnClr[1]                                 ;
              FONT aFnt[1] SIZE aFnt[2] BOLD  VERTICAL                                  ;
   ...
   ENDIF
    ...
    IF aIcon[1] == NIL .OR. aIcon[1] == ""
       nWIco := 0
    ELSE
       nWIco := nSizeIcon + 5
    ENDIF

    //? "************", ProcNL(), cCapt, AT(CHR(13), cCapt) == 0
    IF AT(CHR(13), cCapt) == 0  // одна строчка
       nWCap := GetTxtWidth(cCapt, aFnt[2], aFnt[1], aFnt[3] )
       // ?? "nWCap=", nWCap, "nWIco=", nWIco, ">", "w=", w
       //IF nWCap + nWIco > w
          // Вот здесь как обрезать слово: Металлоконструкции - обрезать до ширины кноки "Металло.."
         // This.&(cObj).Caption := SUBSTR( cCapt, 1, 12 ) // + ".."
       //ENDIF
    ENDIF
   ...
   //                           _LogFile(.T.,ProcNL()+" $$$$$ =>", This.Name, This.Cargo, ThisWindow.Name, This.Cargo:Post) ,;
...
FUNCTION Draw_BtnEx( y, x, oBtn, nWBtn, nHBtn, nGBtn, lRow )
   ...
   DEFAULT oBtn:aPst     := {}     // не использую
   DEFAULT lRow          := .T.    // кнопки по горизонтали
   DEFAULT oBtn:lFntBig  := .F.    // НЕТ пересчёта фонтов для всех кнопок
   DEFAULT oBtn:lVert    := .T.    // вертикальный текст на кнопке
   ...
   ELSE
      ...
      cMsg += REPL("; ", 10)
      SET MSGALERT BACKCOLOR TO { 255, 178, 178 }  // красный
      SET MSGALERT FONTCOLOR TO MAROON
   ...
   IF oBtn:lFntBig   // пересчёт фонтов для всех кнопок

   ENDIF

   cMsg := ""
   IF !IsNumeric(y)  ; cMsg += "ERROR ! y == Nil;"
   ENDIF
   IF !IsNumeric(x)  ; cMsg += "ERROR ! x == Nil;"
   ENDIF
   IF !IsNumeric(w)  ; cMsg += "ERROR ! w == Nil;"
   ENDIF
   IF !IsNumeric(h)  ; cMsg += "ERROR ! h == Nil;"
   ENDIF
   IF LEN(cMsg) > 0
      cMsg += ";" + ProcNL() + ";" + ProcNL(1)
      SET MSGALERT BACKCOLOR TO { 255, 178, 178 }  // красный
   ...
   FOR EACH cObj, cCap, cIco, aClr, lBlk, w IN o:aObj, o:aCap, o:aIco, o:aClr, o:aBlk, o:aWBtn
       i := hb_EnumIndex(cObj)
       //? i, cObj, cCap, x, w, cIco, aFntClr, oBtn:aFnt, nPst, lBlk
...
///////////////////////////////////////////////////////////////////////////////
// Кнопки без градиента / Buttons without gradient
FUNCTION my3DrawButton(nRow, nCol, nWBtn, nHBtn, aBtn)
   LOCAL cObj, cCapt, cFont, nFSize, lBold, cTtipt, aBClr1, aBClr2
   LOCAL aFClr1, aFClr2, hIco1, hIco2, bAction
   LOCAL cForm := _HMG_ThisFormName

   //? ProcNL() ; ? HB_ValToExp(aBtn)
   // {"_Search", e"Поиск\r\nоператора", {107809011, 33751319}, {{109, 1, 1}, {255, 255, 255}, {255, 255, 0}, {128, 128, 128}}, {{255, 255, 255}, {0, 0, 0}, {255, 0, 0}, {0, 0, 0}}, {"DejaVu Sans Mono", 9, .F.}, ""}
   cObj    := aBtn[1]
   cCapt   := aBtn[2]
   hIco1   := aBtn[3,1]
   hIco2   := aBtn[3,2]
