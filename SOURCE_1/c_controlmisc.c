...
HB_FUNC( KEYBD_EVENT )
{
   ...
   keybd_event( bVk, bScan, bUp ? KEYEVENTF_KEYUP : 0, 0 );
}
...
HB_FUNC( _HMG_SETVSCROLLVALUE )
{
   ...
   if( hWnd )
   {
      SendMessage( hWnd, WM_VSCROLL, MAKEWPARAM( SB_THUMBPOSITION, hb_parni( 2 ) ), 0 );
   }
}
...
HB_FUNC( _HMG_SETHSCROLLVALUE )
{
   ...
   if( hWnd )
   {
      SendMessage( hWnd, WM_HSCROLL, MAKEWPARAM( SB_THUMBPOSITION, hb_parni( 2 ) ), 0 );
   }
}
...
{
   HWND     hWnd = hmg_par_raw_HWND( 1 );
   HBITMAP  hBmp = hmg_par_raw_HBITMAP( 2 );
   int      nW = hb_parni( 3 );
   int      nH = hb_parni( 4 );

   hb_retl( CreateCaret( hWnd, hBmp, nW, nH ) );
}
 ...
 * Purpose: Internal HMG helper to position buttons inside/adjacent to textbox-like controls.
 * Parameters:
...
HB_FUNC( MOVEBTNTEXTBOX )
{
   ...
   BOOL  fShowBtn2 = hb_parl( 4 );
   int   nBtnWidth = hb_parni( 5 );
   int   nWidth = hb_parni( 6 );
   int   nHeight = hb_parni( 7 );

   /* System metrics for proper button sizing and spacing */
   int   nMinBtnWidth = GetSystemMetrics( SM_CYSIZE ) - 1;
   int   nSpacing = 2 * GetSystemMetrics( SM_CXEDGE );
   int   nPadding = GetSystemMetrics( SM_CYEDGE );
   int   nBtn2Width, nButtonHeight, nBtn1X, nBtn2X;

   /* Enforce minimum button width */
   if( nBtnWidth < nMinBtnWidth )
   {
      nBtnWidth = nMinBtnWidth;
   }

   /* Disable button if handle is invalid */
   if( !hBtn1 )
   {
      nBtnWidth = 0;
   }
   ...
   nBtn2Width = ( fShowBtn2 && hBtn2 ) ? nBtnWidth : 0;

   nButtonHeight = nHeight - nPadding;
   nBtn1X = nWidth - nBtnWidth - nSpacing;
   nBtn2X = nWidth - nBtnWidth - nBtn2Width - nSpacing;

   /* Resize edit control area */
   SetWindowPos( hEdit, NULL, 0, 0, nWidth, nHeight, SWP_FRAMECHANGED | SWP_NOMOVE | SWP_NOACTIVATE | SWP_NOZORDER );

   /* Position first button */
   if( hBtn1 )
   {
      SetWindowPos( hBtn1, NULL, nBtn1X, -1, nBtnWidth, nButtonHeight, SWP_NOACTIVATE | SWP_NOZORDER );
   }

   /* Position second button if visible */
   if( fShowBtn2 && hBtn2 )
   {
      SetWindowPos( hBtn2, NULL, nBtn2X, -1, nBtn2Width, nButtonHeight, SWP_NOACTIVATE | SWP_NOZORDER );
   }
}
