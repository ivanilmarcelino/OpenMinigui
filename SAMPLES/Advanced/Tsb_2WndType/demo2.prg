 ...
 * Все операции в программе (открытие баз, проверки) после запуска окна MAIN
 * Working with MAIN, STANDARD, MODAL windows and TsBrowse tables.
 * How to remove "freezing/hanging" of the program (loss of focus between MODAL windows)
 * Correct setting of the operating mode of nested MODAL windows
 * Disabling entry into the program if there is a semaphore file demo.stop
 * All operations in the program (opening databases, checks) after starting the MAIN window
*/
#define  _HMG_OUTLOG
...
FUNCTION Main()
   LOCAL cForm := "wMain", aRun //, aMsg, cMsg, i, nMax
   LOCAL nY, nX, nH, nW, nG, owc, oac := App.Cargo, o
   ...
   DEFINE WINDOW &cForm AT nY, nX WIDTH nW HEIGHT nH        ;
          ...
          NOSHOW                                            ;  // we launch the window in HIDE mode
      ...
      owc:aRun     := aRun          // list of program initialization functions
      ...
      @ owc:aIcon[1], owc:nX LABEL Label_Info VALUE App.Cargo:cInfo WIDTH nW - owc:nX HEIGHT 190 TRANSPARENT
      ...
      WITH OBJECT o
       :Event( 0, {|ow,ky|  // ON INIT
                   Local aMsg, cMsg, nMsg, cw, a, aRun := ow:Cargo:aRun
                   Local cFile := ChangeFileExt( App.ExeName, ".stop" )
                   Local lRet, oac := App.Cargo, aLbl, cLbl
                   ow:Cargo:lInitEnd := hb_FileExists( cFile )
                   ? "==>> " + ProcName() + "(" + HB_NtoS(ProcLine()) + ")"
                   ?? "---[ "+ow:Name+":Event("+hb_ntos(ky)+") ]---"
                   ? Repl(".", 5), "=> ON INIT <=", ow:Name, ow:Cargo:lInitEnd
                   If ow:Cargo:lInitEnd   // program stop semaphore file
                      cMsg := "A T T I N G !;;The program is closed for work!;"
                      cMsg += "Wait a while and try to start again"
                      //WaitWinStop( blk, oac:cIcoDef, 96, cMsg, WHITE, BLUE, 15 /*seconds*/ )
                      _wSend(200, ow:Name, {oac:oBloks:bWait0, cMsg})
                      ow:Release()
                   Else
                      SET WINDOW THIS TO ow:Name        // !!! NECESSARILY
                      aMsg := {"Initialization of the program", App.Exename, "...."}
                      //cw   := WaitWindow( aMsg, bOpn, 600, 16, NIL, WHITE, PURPLE )
                      cw   := WaitWindow( aMsg, .T., 600, 16, NIL, WHITE, PURPLE )
                      EnableWindow( ow:Handle )
                      aLbl := HMG_GetFormControls(cw, "LABEL")  // list of objects
                      cLbl := ATail(aLbl)
                      For EACH a IN aRun
                          cMsg := a[1]
                          SetProperty( cw, cLbl, "Value", cMsg )
                          DO EVENTS
                          wApi_Sleep( 200 )
                          lRet := EVal(a[2]) ; Default lRet := .F.   // !!! function start
                          ? hb_EnumIndex(a), a[1], lRet, cLbl, cMsg
                          If !lRet    
                             AlertStop("ERROR !;" + a[1],,,64,{RED})
                             nMsg := 99
                             EXIT
                          Endif
                      Next
                      WaitWindow()
                      SET WINDOW THIS TO               // !!! NECESSARILY
                      ow:Show()                        // !!! show main window on screen
                      This.Topmost := .T.              // !!! NECESSARILY
                      SendMessage( ow:Handle, WM_PAINT, 0, 0 )
                      ow:SetFocus(ow:Cargo:cFocus)
                      DO EVENTS
                      This.Topmost := .F.
                      IF !Empty(nMsg) ; _wPost(nMsg, ow:Name)
                      ENDIF
                   Endif
                   Return Nil
                   })
       ...
       :Event(200, {|ow,ky,ap|                   // Wait
                     Local oac := App.Cargo, msg, blk, siz, fcl, bcl, sek
                     Default ap := {}
                     ? "===>>>",ky, ap, ow:Name
                     ASize(ap, 7)
                     blk := ap[1]
                     ...
                     bcl := ap[5]
                     sek := ap[6]
                     Default blk := oac:oBloks:bWait0
                     Default msg := "A T T E N T I O N !;;Wait for work to be completed.;;"
                     Default siz := 96        // Icon size
                     Default fcl := WHITE     // FColor
                     Default bcl := BLUE      // BColor
                     Default sek := 15        // seconds
                     WaitWinStop( blk, oac:cIcoDef, siz, msg, fcl, bcl, sek )
                     ky := ow
                     Return Nil
                     })
      END WITH
...
FUNCTION Set_Default_Tsb_Param()
...
FUNCTION Set_Default_Tsb_Report()
   LOCAL oac := App.Cargo, o
   // Calculation default
...
FUNCTION Table_Three(oWnd, cBtn, lAls)
   ...
   DEFINE WINDOW &cForm AT nY, nX WIDTH nW HEIGHT nH           ;
      ...
      IF owc:lAlias
         ButtonBar({"Modal", "Wait new", "Exit"}, cNam, , , App.Object:W2)
      ELSE
      ...
      WITH OBJECT This.Object
         ...
         :Event( 1, {|ow,ky,cn|
            ...
            IF owc:lAlias
         :Event( 2, {|ow,ky,cn|
                      Local hm := _HMG_InplaceParentHandle
                      ky := _WindowObj(_HMG_MainHandle)
                      SET WINDOW THIS TO ow:Name
                      _HMG_InplaceParentHandle := ow:Handle
                      _wSend(200, ky:Name, {})
                      _HMG_InplaceParentHandle := hm
                      SET WINDOW THIS TO
                      ow:Enabler(cn, .T.)
                      ow:SetFocus(ow:Cargo:cFocus)
                      DO EVENTS
...
FUNCTION WaitWinStop(bBlock, cIcon, nISize, cText, aFClr, aBClr, nTime)
   LOCAL hFont, aFont, cFont, nFSize, aWHTxt, cMsg, cFocus, owc
   LOCAL nY, nX, nH, nW, nG, cInfo
   LOCAL cForm  := "MG_Wait_Stop"
   LOCAL cTitle := cForm + "_" + "Window"

   hFont  := GetFontHandle("DlgFont")
   aFont  := GetFontParam(hFont)
   cFont  := aFont[1]
   nFSize := aFont[2]
   aWHTxt := WHTextSize( cText, cFont, nFSize )
   cMsg   := aWHTxt[3]
   nG     := 20
   nW     := nG + nISize + nG + aWHTxt[1] + nG * 2
   nH     := nG + aWHTxt[2] + nG + nFSize*2 + nG * 2
   cInfo  := "Closing window via " // секунд
   cFocus := "Buff"
   ? PROCNAME()+"()", HB_ValToExp(aWHTxt)
   //_HMG_InplaceParentHandle := oWnd:Handle
   ...
   DEFINE WINDOW &cForm AT nY, nX WIDTH nW HEIGHT nH    ;
      ...
      owc:nTime   := nTime    // кол-во секунд счёта
      owc:nFor    := 1
      owc:cInfo   := cInfo
      // owc:oParent := oWnd
      owc:cFocus   := cFocus
      ...
      owc:lEscape  := .F.
      owc:lBreak   := .F.
      owc:lBlock   := IsBlock( bBlock )
      owc:cLblInfo := "Label_2"              //!!!
      ...
      //!!!
      @ nY, 0 LABEL &(owc:cLblInfo) WIDTH nW HEIGHT nFSize*2 VALUE cInfo ;
      ...
      WITH OBJECT This.Object
         :Event( 0, {|ow| ow:SetFocus(ow:Cargo:cFocus), _wPost(1) } )
         :Event( 1, {|ow,ky,cn|
                     ...
                     DO EVENTS
                     ?? owc:nFor, ">", owc:nTime
                     IF owc:nFor > owc:nTime .or. owc:lBreak
                        //ThisWindow.Cargo:lBreak  := .T.
                        owc:lBreak  := .T.
                        owc:lEscape := .T.
                        ?? "_wPost(99)", ThisWindow.Cargo:lBreak
                        _wPost(99)
                     ELSE
                        ow:Enabler(cn, .T.)
                     ENDIF
...
FUNCTION WHTextSize( cMsg, cFont, nFSize )
...
*----------------------------------------------------------------------------*
   ...
   oac:cInfo     += "!!! Terminate the program when the semaphore file demo.stop is present" + CRLF + CRLF + PROGVER
   ...
   // block code for working in WaitWinStop() function
   oac:oBloks := oHmgData()
   oac:oBloks:bWait0 := {|ow,ky,cn|
                   Local cv, owc := ow:Cargo
                   Local cInf := owc:cLblInfo       //!!!
                   Default cn := owc:cTimer
                   ow:Enabler(cn, .F.)
                   //ky := This.&(cn).Cargo + 1
                   owc:nFor ++
                   cv := HB_NtoS( owc:nTime - owc:nFor )
                   ? ProcName(), "oac:oBloks:bWait0", ow,ky,cn
                   This.&(cInf).Value := owc:cInfo + cv + " sec."
                   DO EVENTS
                   ?? owc:nFor, ">", owc:nTime
                   IF owc:nFor > owc:nTime .or. owc:lBreak
                      //ThisWindow.Cargo:lBreak  := .T.
