...
Local a_Items[20][2], a_head[2], a_width[2], bColor
...
bColor := {|x, nItem| iif( getCheckboxState( nItem ), RGB(220,220,220), RGB(255,255,255) ) }
...
DEFINE WINDOW Form_1 ;
   ...
   @ 50,50 GRID Grid_1 ;
     ...
     MULTISELECT ;
     CHECKBOXES ;
     ON CHECKBOXCLICKED {|nSpace, nRow| checkboxesRefresh( nSpace, nRow ) }
...
*--------------------------------------------------------*
function getCheckboxState(i)
return ( Form_1.Grid_1.CheckboxItem (i) == .T. )

*--------------------------------------------------------*
Procedure checkboxesRefresh( nSpace, nRow )

if _ListViewItemChecked( GetControlIndex("Grid_1", "Form_1") ) .OR. nSpace > 0
  Form_1.Grid_1.DisableUpdate
  Form_1.Grid_1.Refresh
  Form_1.Grid_1.EnableUpdate
endif
if nSpace == 0
  Form_1.Title := "Row On CheckBox Clicked: " + hb_ntos(nRow)
endif

Return

*-----------------------------------------------------------------------------*
STATIC FUNCTION _ListViewItemChecked( i )
*-----------------------------------------------------------------------------*
