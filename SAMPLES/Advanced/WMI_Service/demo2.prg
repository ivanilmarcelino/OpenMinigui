...
FUNCTION MemoryInfo()
   ...
   FOR EACH oMemory IN oWmi:ExecQuery( "SELECT * FROM Win32_OperatingSystem" )

      cInfo+= "Memory Total: " +  XTOC(Round(Val(oMemory:TotalVisibleMemorySize)/(1024*1024),2)) + " GB"+ CRLF
      cInfo+= "Memory Virtual: " +  XTOC(Round(Val(oMemory:TotalVirtualMemorySize)/(1024*1024),2)) + " GB"+ CRLF
...
FUNCTION BiosInfo()
   ...
   FOR EACH oBios IN oWmi:ExecQuery( "SELECT * FROM Win32_BIOS" )
      ...
      cInfo+="Date: " +  XTOC(StoD(Left(oBios:ReleaseDate,8)))+ CRLF
...
FUNCTION VideoInfo()
   ...
   FOR EACH oVideo IN oWmi:ExecQuery( "SELECT * FROM Win32_VideoController" )
      ...
      if oVideo:VideoModeDescription != NIL
...
FUNCTION SoundInfo()
   ...
   FOR EACH oSound IN oWmi:ExecQuery( "SELECT * FROM Win32_SoundDevice" )
      ...
      if IsWinNT()
         ...
         cInfo+="Manufacturer: " +  oSound:Manufacturer+ CRLF
...
FUNCTION OSInfo()
   ...
   FOR EACH oOs IN oWmi:ExecQuery( "SELECT * FROM Win32_OperatingSystem" )
      ...
      cInfo+="Architecture: " +  oOs:OSArchitecture+ CRLF
      cInfo+="Version: " +  oOs:Version+ CRLF

      if !Empty(oOs:CSDVersion)

         cInfo+="Service Pack: " +  XTOC(oOs:CSDVersion)+ CRLF

      endif

      cInfo+="Build: " +  oOs:BuildNumber+ CRLF+ CRLF
      cInfo+="Locale: " +  oOs:Locale+ CRLF
      cInfo+="Serial Number: " +  oOs:SerialNumber+ CRLF
      cInfo+="Registered User: " +  oOs:RegisteredUser+ CRLF+ CRLF
      ...
      if IsWinNT()
         ...
         cInfo+="Install Date: " + XTOC(StoD(Left(oOs:InstallDate,8)))+ CRLF
         cInfo+="Last BootUp: " + XTOC(StoD(Left(oOs:LastBootUpTime,8)))+ CRLF
...
STATIC FUNCTION GetProductType( nType )
...
FUNCTION EnvVarInfo()
   ...
   FOR EACH oEnvVar IN oWmi:ExecQuery( "SELECT * FROM Win32_Environment" )
      ...
      cInfo += oEnvVar:Name + "=" + oEnvVar:VariableValue+ CRLF
