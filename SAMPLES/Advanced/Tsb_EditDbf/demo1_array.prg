...
FUNCTION ArrayLoadDim()
   ...
   // заполняется для вложенного справочника "CALC", для других не надо
   oSrkZa := oHmgData()  
   oSrkZa:aFld  := {"Ksrokza","DateSrok" }  // поля записи в базу / database entry fields                                                                        //     9  10  11   12  13        14        15
   oSrkZa:Help  := "!!! Все значения в функции Tsb_myWinCalc2() / All values in the Tsb_myWinCalc2() function" 
   ...
   oVipZa:aFld  := {"KVipZa", "DATEVip", "TimeVip", "INET" }  // поля записи в базу - "INET" Флаг для передачи на сайт
