...
#xtranslate LenU( <c> ) => Len( <c> )
/*
#xtranslate LenU( <c> ) => iif( hb_IsArray( <c> ) .or. hb_IsHash( <c> ), Len( <c> ), iif( hb_IsChar( <c> ), hb_ULen( <c> ), 0 ) )
#xtranslate At( <c>, <n> )             => hb_UAt( <c>, <n> )
#xtranslate Left( <c>, <n> )           => hb_ULeft( <c>, <n> )
#xtranslate Right( <c>, <n> )          => hb_URight( <c>, <n> )
#xtranslate SubStr( <cS>, <nS> )       => hb_USubStr( <cS>, <nS> )
#xtranslate SubStr( <cS>, <nS>, <nL> ) => hb_USubStr( <cS>, <nS>, <nL> )
#xtranslate Subs( <cS>, <nS> )         => hb_USubStr( <cS>, <nS> )
#xtranslate Subs( <cS>, <nS>, <nL> )   => hb_USubStr( <cS>, <nS>, <nL> )
*/
...
FUNCTION Main()
   ...
   LOCAL nSize := 12
   LOCAL cForm := "wMain"
   LOCAL cIni  := hb_FNameExtSet( hb_ProgName(), ".ini" )
   LOCAL oIni  := TIniData():New(cIni, .T.):Read()
   LOCAL oCom  := oIni:COM
   ...
   DEFINE WINDOW wMain AT nY, nX WIDTH nW HEIGHT nH ;
      TITLE "MiniGUI TsBrowse Demo. Ini value => _Tbrowse()" ;
...
FUNCTION oTsb_Init( oTsb )
   ...
   oTsb:bBody := {|ob,op|
        ...
        lEditNo := HB_ISARRAY(op:aEditNo) .and. LenU(op:aEditNo) > 0
