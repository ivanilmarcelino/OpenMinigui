   ...
   DO WHILE .T.
      ...
      if nDevice > 0
         ...
         elseif nDevice == DEV_PDFFILE
            ...
            PSSetImageQuality( 40 )
         elseif nDevice == DEV_PRINTER .and. File("Test" + AllTrim(Str(nFile-1, 3, 0)) + ".pdf")
            PSPrintPDF( "Test" + AllTrim(Str(nFile-1, 3, 0)) + ".pdf", .T. )
         endif

         if nChoice != 21
            if nDevice == DEV_PDFFILE
               Eval(aProc[nChoice])
               if nChoice != 3 .and. nChoice != 24
                  ShellExecute( 0, "open", "viewer.exe", "Test" + AllTrim(Str(nFile, 3, 0)) + ".pdf", , 1 )
               elseif nChoice == 24
                  ShellExecute( 0, "open", "Encrypted PDF document.pdf", , , 1 )
               endif
            endif
         else
            Eval(aProc[nChoice])
...
PSBeginDoc(, "Selected printer capabilities", APS_PORTRAIT)
   ...
   @0, 39 TEXTOUT "Capabilities of " + PSGetPrinters()[1] JUSTIFY APS_CENTER STYLE APS_BOLD POINT 14
...
/*-----------------------------------------------------------------------------
Function ...: AbeeExportPDF2Txt()
Description : 
Author .....:
Date .......: January 2022
-----------------------------------------------------------------------------*/
Static Function AbeeExportPDF2Txt()

if File("Test1.pdf")
   PSExport2Txt("Test1.pdf", "output.txt")
   HMG_SysWait()
   ShellExecute( 0, "open", "output.txt", , , 1 )
endif
...
FUNCTION Achoice(t, l, b, r, aItems, cTitle, nValue)
    ...
    t := l := b := r := Nil
    DEFAULT cTitle TO "Please, select", nValue TO 3
