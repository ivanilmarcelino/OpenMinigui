...
#include "minigui.ch"
//////////////////////////////////////////////////////////////////////////////////
FUNCTION Form_ExportFile(oWnd, cTitle, cMsg, cFileExp, cIco3, lSayCdPg, cSayCP)
   LOCAL cIcon, cFont, nFontSize, aBackColor, aBColorTxt, nI
   LOCAL nY, nX, nW, nH, nG, nWTxt, nHTxt, nHBtn, nWBtn, nYBtn, nXBtn
   LOCAL nX2, ao, owc, oDlu, nWTxt2, lOpen, cPath, aBtnBClr, lDelSkip 
   ...
   cCdPg      := "RU1251" 
   cCdPg      := IIF( LEN(cSayCP)>0, cSayCP, cCdPg)
   lOpen      := .T. 
   lDelSkip   := .T.
   ...
   DEFINE WINDOW &cForm AT nY, nX WIDTH nW HEIGHT nH ;
      ...
      nY += This.Chkl_1.Height + nG

      cVal  := IIF( ao:cLang == "RU", "ѕропускать удаленные записи",;
                      "Skip deleted entries" )
      nWTxt := GetTxtWidth( cVal, nFontSize + 3, cFont ) + 90
      @ nY, nX CHECKLABEL Chkl_2 WIDTH nWTxt HEIGHT 34    ;
        VALUE cVal LEFTCHECK SIZE nFontSize + 3           ;
        IMAGE { 'CheckT32', 'CheckF32' }                  ;
        ON MOUSEHOVER Rc_Cursor( "MINIGUI_FINGER" )       ;
        BACKCOLOR aBackColor                              ;
        ON INIT {|  | This.Checked := lDelSkip }          ;
        ONCLICK {|ow| This.Checked := ! This.Checked,; 
                      lDelSkip  := This.Checked ,;
                      ow := ThisWindow.Object , ow:Cargo:lDelSkip := lDelSkip  }
      ...
      aBtn  := { "Button_Open", cTxt, "iDBase64", "iDBase64x2", nHIco, aBtnFClr, aBtnFont, "" }
   ...
   ? "@@@@@ =>", ProcNL(), oWnd, valtype(oWnd), oWnd:ClassName, oWnd:Name
   ?? oWnd:Cargo
   ?? oWnd:Cargo:aFileExport

RETURN NIL                      // ничего не вернуть 
//RETURN oWnd:Cargo:aFileExport // вернуть - вариант 2
   ...
   WITH OBJECT This.Object
     ...
     :Event( 3, {|ow| // кнопка - выбор кодовой страницы
                      ...
                      If LEN(aR) > 0
                         ...
                         //This.Button_CdPg.Icon  := aR[4]  - нет смены иконок
                      Endif
                      ow:SetFocus('Label_0')
                      Return Nil
                      } )
     ...
     :Event(90, {|ow| // Ёкспорт в файл и закрыть окно
                      ...
                      cTtl := IIF(ao:cLang == "RU", cTtl, cTtl2 )
                      If LEN(owc:cFile) > 0
                         aR := { owc:cPath + "\" + owc:cFile, owc:cCdPg, owc:lOpen, owc:lDelSkip } // исправили
