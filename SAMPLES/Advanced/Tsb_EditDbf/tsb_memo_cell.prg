...
FUNCTION CellEditMemo(uVal, oBrw)   // создать окно редактировани€ memo
   LOCAL aLang, cFont, nFSize, hFont, aFont, cForm, cFType, nFLn, o
   LOCAL nW, nH, nCol, oCol, aBrw, oCell, nWCell, nHCell
   ...
   aLang := { "F2-write", "Esc-exit" }
#else
   aLang := { "F2-запись", "Esc-отмена" }
#endif
   ...
   DEFINE WINDOW &cForm AT nY + nHCell, nX-5 CLIENTAREA nW-5, nH ;
      ...
      nWBtn := 200
      // ширина кнопки = ... + длина текста кнопки
      nWBtn := 32 + 10 + GetTxtWidth( REPL("H",12) , nFSize, cFont, .T. ) + 50
      IF nWBtn*2 + nG*3 > nW-5
         nWBtn := ( nW - 5 ) / 3
      ENDIF

      nHBtn := 32 + nG + IIF( nFSize > 16, nFSize, 0 )
      ? ProcNL(), "%%%%%%%% ширина кнопки=" , nWBtn, "высота кнопки=", nHBtn
      nHIco := IIF( nFSize > 16, nHBtn-nG*2, 32 )
      ?? "высота иконки=", nHIco
