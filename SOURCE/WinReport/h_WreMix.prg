...
*-----------------------------------------------------------------------------*
         ...
         if "LAND" $ Str1 ;(landscape:=.t.,OwR:aStat[ 'Orient' ] := 2); Else;landscape:=.F. ; Endif
   ...
   do case
      ...
      case ascan(ArryPar,[PRINT])=3 .OR. ascan(ArryPar,[SAY])= 3
              ...
              otherwise
                  ...
                  _definefont("XTX", _ARG3 , _ARG4 )

                  _arg5:= ( Getfontheight( "X" ) * .355 ) / 2
                  *_arg5 := (_arg4*1.5875)/6
                  RELEASE FONT "XTX"

                  if ascan(arryPar,[TO]) > 1  //require multiline

                     _HMG_PRINTER_H_MULTILINE_PRINT( if(MGSYS,_HMG_SYSDATA [ 374 ],_hmg_printer_hdc) ;
                     , if([LINE]$ Arrypar[1],&(Arrypar[1])-_arg5 + (mncl/3) -oWr:aStat[ 'Hbcompatible' ] ,eval(epar,ArryPar[1])-_arg5 + (mncl/3) -oWr:aStat[ 'Hbcompatible' ]) ;
                     , eval(epar,ArryPar[2])  ;
