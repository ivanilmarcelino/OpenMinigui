...
FUNCTION Form4Filter(oWnd, ky, cn, oBrw)
   ...
   DEFINE WINDOW &cForm AT nY, nX WIDTH nW HEIGHT nH   ;
      ...
      oTsb:bBody  := {|ob,op| myTsbKeyFX(ob,op), myTsbEdit(ob,op), myTsbSizeCol(ob)   }  // другие настройки тсб  -> см.ниже
...
STATIC FUNCTION CreateConfigFilter(cDbf,oBrw)
   ...
   AAdd( aStr, { 'FVAL1' , 'C', 35, 0 } )    //"YYYY-MM-DD hh:mm:ss.fff"
   AAdd( aStr, { 'ORAND1', 'C',  5, 0 } )
   AAdd( aStr, { 'FZNK2' , 'C',  2, 0 } )
   AAdd( aStr, { 'FVAL2' , 'C', 35, 0 } )
   AAdd( aStr, { 'ORAND2', 'C',  5, 0 } )
   AAdd( aStr, { 'FZNK3' , 'C',  2, 0 } )
   AAdd( aStr, { 'FVAL3' , 'C', 35, 0 } )
...
STATIC FUNCTION myTsbInit( oBrw, oTsb )  // настройки
   ...
   WITH OBJECT oBrw
      ...
      :nMemoHV       :=  1           // показ строки мемо-поля
      :lNoHScroll    := .T.          // убрать HScrollBar
...
///////////////////////////////////////////////////////////////////////////////////
// ДЕЛАЕМ ДО END TBROWSE
STATIC FUNCTION myTsbSizeCol( oBrw )
   LOCAL cBrw, nTsb, nW1Col, nW2Col, nWDel, nWZnk, nWAnd, aCol, nWTsb, nWSum
   LOCAL nI, nWCol, oCol, nWLen, nLen, nCol, hFont, nW3Col
   ...
   IF nCol > 0
      ...
      nW1Col := GetTextWidth( Nil, REPL("0", nLen + 2), hFont )   // кол-во знаков + 2 знака
      oCol:nWidth := nW1Col                                       // новая ширина
   ENDIF
   ...
   nWTsb := This.&(cBrw).ClientWidth
   nWLen := oBrw:GetAllColsWidth() - 1
   ...
   */
   ...
   nTsb  := oBrw:GetAllColsWidth() - 1
   nWTsb := This.&(cBrw).ClientWidth
   ? "####", ":GetAllColsWidth()=", nTsb, "nWTsb=", nWTsb
   nWSum := GetVScrollBarWidth() + nW1Col + nW2Col + nW3Col + nWAnd*2 + nWDel + nWZnk*3
   nWCol := INT( ( nWTsb - nWSum ) / 3 )
   ? "####", "nWTsb=",nWTsb, "nWSum=",nWSum, "nWCol=",nWCol
   ? "####", "(+)nWTsb=", nWCol*3 + nWSum
...
STATIC FUNCTION myTsbEnd( oBrw, oTsb )
   LOCAL nBCSpH, nI, cCol, oCol, a4Clr, nTest, nLen, nCol
...
STATIC FUNCTION myTsbEditPrev( uVal, oBrw )
   ...
   ELSEIF c1Typ $ "CMQ" .OR. c1Typ == "C:U" .OR. c1Typ == "C:B" .OR. ;
...
STATIC FUNCTION FilterDbf(owc)   // создание фильтра
   ...
   IF LEN(aDim) > 0
      ...
      FOR nI := 1 TO LEN(aDim)
         ...
         IF cTyp $ "CMQ" .OR. cTyp == "C:U" .OR. cTyp == "C:B" .OR. cTyp == "M:U" .OR. ;
            cTyp == "W" .OR. cTyp == "P"
