...
FUNCTION Menu3Table(ow,ky,cn)
   ...
   LOCAL xRet, cForm, aFntExt, aTable, cLang, cLang2, cIcon, aBtn3
   LOCAL nLen, cMsg, nY, nX

   ? ProcNL(), ow:Name,ky,cn
   cForm := ow:Name
   cIcon := "iAccess32"

   // записать список таблиц из файла Access
   ...
   FOR nI := 1 TO LEN(aTable)
      AADD( aMenu, { cIcon, cMsg + aTable[nI] , .F.  , "msgDebug"  , "Str"+HB_NtoS(nI) , nI } )
   NEXT
   AADD( aMenu, {                                                       } )
   ...
   IF nChoice > 0
      ...
      ELSE
        xRet := OpenTableOne(ow,nPos,aMenu[nChoice])
      ENDIF
   ENDIF
   DO EVENTS
...
/////////////////////////////////////////////////////////////////////////////////////////////////
FUNCTION OpenTableOne(ow,nPos,aMenu)
    LOCAL cIco, cTtl, lWin1, cForm

    cTtl  := aMenu[2]
    cIco  := aMenu[1]
    lWin1 := .T.       // одно окно и остаться на окне
    cForm := Tsb_ViewAccess(ow, nPos, cIco, cTtl, lWin1)  // -> tsb_ViewAccess.prg

RETURN NIL

/////////////////////////////////////////////////////////////////////////////////////////////////
FUNCTION OpenTableAll(ow,cIcon)
   LOCAL aTable, nI, cTtl, lWin1, cForm, aForm, hForm
   ...
   FOR nI := 1 TO LEN(aTable)
      cTtl  := aTable[nI]
      cForm := Tsb_ViewAccess(ow, nI, cIcon, cTtl, lWin1)  // -> tsb_ViewAccess.prg
      IF LEN(cForm) > 0
         AADD( aForm , cForm )
      ENDIF
   NEXT
