...
FUNCTION TestZero()
   LOCAL aRet, aMenu, nI, nL

   nL    := 0
   aMenu := ARRAY(8)
   FOR nI := 1 TO LEN(aMenu)
      aMenu[nI] := HB_NtoS(nI) +  " - Selection menu "
      nL        := MAX( LEN(aMenu[nI]), nL)
   NEXT

   FOR nI := 1 TO LEN(aMenu)
      aMenu[nI] := PADR(aMenu[nI],nL)
   NEXT

   IF App.Cargo:lDebug  ; ? ProcNL(), "aMenu=", aMenu ; ?v aMenu 
   ENDIF
...
FUNCTION TestAchoice(nMode,cMenu)
   ...
   LOCAL aMenu3, aIcon3, aBackClr, aParIco3, lTxtAlign, aColor
   ...
   ELSEIF nMode == 9                  // Большое меню без иконок
      aParMenu[3] := 2                       // отступ между кнопками
   ...
   ELSEIF nMode == 11                   // Large menu with icon
      ...
      // параметры кнопки Exit
      aBtnFClr    := { RED      , YELLOW    }   // цвет фонта кнопок
      aBtnBClr    := { aColor   , BLACK     }   // цвет фона кнопок
      aBtnGrad    := { CLR_VK   , CLR_WHITE }   // цвет градиента кнопок
      aIcoExit    := { {"iExit1", "iExit2"}, "Выход / Exit", aBtnFClr, aBtnBClr, aBtnGrad }   // иконки для выхода
      aParIco[2]  := aIcoExit
      aParIco[3]  := { aColor, RED }           // цвет фонта кнопок
      nWinAlign   := 1   // 1-Left Align Icon, 2-Right Align Icon
      aIcoWin     := { "iHP1", 72, 0, 20, nWinAlign }             // иконка на форме
      aRet        := MenuAchoice( TestDim3(), aParWin, aParMenu, aParIco, aIcoWin, lTxtAlign )

   ELSEIF nMode == 12                   // B. Menu by array (default)
      aRet       := MenuAchoice(aMenu)

   ELSEIF nMode == 13                 // Меню по массиву c иконкой справа на форме
      nWinAlign := 1   // 1-Left Align Icon, 2-Right Align Icon
      aIcoWin   := { "iHP1", 150, 0, 20, nWinAlign }             // иконка на форме
      aParWin   := { "Form_Print", cTitle, "iHP1", aBackClr, 80, 150, nW, nH }
      aRet      := MenuAchoice(aMenu3, aParWin, , , aIcoWin)

   ELSEIF nMode == 14                  // Array menu with icon on the right side of the form + gradient
   ...
   ELSEIF nMode == 15                  // Меню дней недели (выровнять текст по левому краю)
      lTxtAlign  := 1        // Align Text: 0-None, 1-Left, 2-Right, 3-Center
      aDay := TestaDay()
      aRet := MenuAchoice(aDay, , , , , lTxtAlign)

   ELSEIF nMode == 16                  // Меню дней недели (выровнять текст по правому краю)
   ...
   IF App.Cargo:lDebug  ; ? ProcNL(), "Return result aRet= " + HB_ValToExp(aRet) 
   ENDIF
...
////////////////////////////////////////////////////////////////////////////
STATIC FUNCTION TestDim3()
   LOCAL nI, cTxt, aMenu[10]

   FOR nI := 1 TO 10
      cTxt := "Selection menu-" + HB_NtoS(nI)
      aMenu[nI] := cTxt
   NEXT
...
////////////////////////////////////////////////////////////////////////////
STATIC FUNCTION TestIcon3()
   LOCAL nI, aIcon := {}

   FOR nI := 1 TO 10
      AADD( aIcon, {"iFolder1","iFolder2" } )
   NEXT
