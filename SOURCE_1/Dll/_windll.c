...
HB_FUNC( LOADLIBRARY )
{
   HMODULE hResult = LoadLibraryA( ( LPCSTR ) hb_parc( 1 ) );

   hmg_ret_raw_HANDLE( hResult );
}
...
{
   HMODULE hResult = LoadLibraryExA( ( LPCSTR ) hb_parc( 1 ),
                                      hmg_par_raw_HANDLE( 2 ),
                                      hmg_par_DWORD( 3 )
                                      );
   hmg_ret_raw_HANDLE( hResult );
}
...
HB_FUNC( GETPROCADDRESS )
{
   ...
   if( ( dwProcAddr = ( ULONG ) GetProcAddress( ( HMODULE ) HB_PARNL( 1 ),
                                                ...
                                                ( LPCSTR ) MAKELONG( hmg_par_WORD( 2 ), 0 ) ) ) == 0 )
   {
      if( HB_ISCHAR( 2 ) )
      {
...
HB_FUNC( CALLDLL )
{
   ...
   int       iReturn = HB_ISNIL( 4 ) ? 0 : hb_parni( 4 );
   int       Flags;
   double    DblParms[ 15 ];
   DYNAPARM  Parm[ 15 ];
   HINSTANCE hInst      = hmg_par_raw_HINSTANCE( 1 );
   DWORD     lpFunction = hmg_par_DWORD( 2 );
   RESULT    rc;
         ...
         switch( hb_parni( i - 1 ) )
         {
            ...
            case CTYPE_CHAR:
               ...
               if( HB_ISNIL( i ) )
                  ...
                  Parm[ iCnt ].dwArg = hmg_par_DWORD( i );
               iCnt++;
               break;
