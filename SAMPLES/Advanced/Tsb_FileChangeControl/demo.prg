 ...
 *
 * –абота с классом TThrData (доступ в потоках к переменным идет с блокировками)
 * «апуск программ через hb_ProcessOpen() в отдельном потоке
 * ѕроверка на вторую копию запуска процесса
 *  онтроль изменени€ файлов в своей программе
 * «акрытие процессов запущенных из своей программы
...
#define _HMG_OUTLOG
#define PRG_NAME  "Control file changes in another program / 05.2023 / Timer check"
...
INIT PROCEDURE My_Sets_Env
   LOCAL tTime, cFont := "DejaVu Sans Mono", nSize := 14
   LOCAL cMsg, cLog := ChangeFileExt( App.ExeName, '.log' )
   ...
   // 1-вариант использовани€ переменной дл€ потоков (пример, далее нигде не используетс€)
   // 1st option for using a variable for threads (example, not used anywhere else)
   PUBLIC oThread ; oThread := oThrData()
...
////////////////////////////////////////////////////////////////////////////////
PROCEDURE Main
   LOCAL nG, nY, nX, nW, nH, nHBtn, nWBtn, nRecno, cFormName
   ...
   AFILL( oThread:aProcess  , {} )   // номер записи дл€ блокировани€, если={}, то не было запуска внешнего потока
   ...
   DEFINE WINDOW &cFormName                                      ;
      ...
      This.Cargo:tTime  := HB_DATETIME()
      // в контейнер дл€ работы в thread-ах и основного процесса запоминаем
      // into the container for work in threads and the main process, remember
      App.Cargo:cWindow := This.Name    // им€ окна, просто дл€ примера
      App.Cargo:hWindow := This.Handle  // handle окна, просто дл€ примера
      ...
      DEFINE STATUSBAR FONT cFont SIZE nFSize - 4
         ...
         STATUSITEM "00:00:00"    WIDTH 120  FONTCOLOR GRAY
      END STATUSBAR
      ...
      App.Cargo:oBrw  := oBrw     // object for debug output to the form

      nY += nG + nHTsb

      //  App.Cargo:cWinMemo := "Edit_Memo"     -  должно совпадать
      @ nY, nX EDITBOX Edit_Memo WIDTH nWTsb HEIGHT nFSize*20 VALUE "" ;
         BACKCOLOR SILVER FONTCOLOR BLUE FONT cFont SIZE nFSize - 5 ;
      ...
      WITH OBJECT This.Object
         :Event( 0, {|  | wApi_Sleep(10) , This.Timer_1.Enabled := .T. ,;
                          LogScreen(), DoEvents() } )

         // select row from table
         :Event( 2, {|ow,ky,xv,ob| ob := This.Tsb_File.Object, myRunFiles(ow,ky,xv,ob)  } )
         // variable output
         :Event( 5, {|ow,ky,xv| SetProperty(ow:Name, "Timer_1", "Enabled", .F.) ,;
                                myDebugVar(ow,ky,xv), DoEvents()                ,;
                                SetProperty(ow:Name, "Timer_1", "Enabled", .T.) } )

         :Event(90, {|  | CloseProcess(), DoEvents(), _wSend(91)                                               } )
         :Event(91, {|  | _LogFile(.T., ">>> End of program <<<  "+HMG_TimeMS((App.Cargo):tTime) ), DoEvents() } )
         //:Event(92, {|  | DoEvents(), ShellExecute(0,"Open",cLog,,,1)                             } )
         //:Event(92, {|ow| DoEvents(), ShellExecute(0,"Open",ow:Cargo:cLog,,,1)  /* could be so */ } )
         :Event(99, {|ow| ow:Release()  } )
      END WITH

      // enable timer-2 every 5 seconds
      DEFINE TIMER Timer_1 INTERVAL App.Cargo:nTimerSec * 1000 ;
         ACTION {|| SetProperty(cFormName, "Timer_1", "Enabled", .F.)  ,;
                    Timer1Check(cFormName)                             ,;
                    SetProperty(cFormName, "Timer_1", "Enabled", .T.) }
         This.Timer_1.Enabled := .F.   // disable Timer_1 до On Init
         This.Timer_1.Visible := .F.   // block of code without changing environment This for the duration of Timer_1
...
///////////////////////////////////////////////////////////////////////////////////////
FUNCTION Timer1Check(cForm)  // timer check
   LOCAL cVal, cWnd, nRec
   // This environment does not exist for this window and can only be obtained from the function parameter
   LOCAL oWnd := _WindowObj(cForm)
   LOCAL oBrw := oWnd:Cargo:oBrw
   LOCAL o := App.Cargo

   cWnd := o:cWinForm
   cVal := HMG_TimeMS(App.Cargo:tTime)
   ? cVal , ProcNL(), "App.Cargo:lRefresh=", o:lRefresh
   ?? "Form =", cForm, oWnd:Name, oWnd:Handle, oBrw:cParentWnd, oBrw:cControlName
   oWnd:StatusBar:Say(cVal, 4)
   //SetProperty( cWnd, "StatusBar" , "Item" , 4, cVal ) // could be so

   IF o:lRefresh .and. ! oBrw:IsEdit  // if the file is changed - reread the table
      o:lRefresh := .F.
      nRec := (oBrw:cAlias)->( RecNo() )
      oBrw:Refresh()
      oBrw:GotoRec(nRec)              // eyebrow sync
      ? "Table update success !" ; ?
   ENDIF

   LogScreen()  // read the log-file and output to the form
...
FUNCTION LogScreen()
   ...
   DO EVENTS
   // ¬ потоках (один или много их) нельз€ делать команды hmg
   // SetProperty(...) которые мен€ют внутренние PUBLIC _HMG_... переменные, т.к
   // эти переменные без блокировок доступа, а значит доступны “ќЋ№ ќ дл€ чтени€,
   // теоретически их можно мен€ть, но одновременный доступ из разных потоков на
   // чтение или запись даст что то плохое, т.к. пам€ть, адрес данных могут уже
   // дл€ них оказатьс€ разными.
   // ¬ данном случае можно так делать, т.к. нет в основном потоке
   // действий одновременных с потоком над контролом EDITBOX Edit_Memo.
   // ¬ этом примере notepad, EDITBOX и oBrw работают последовательно,
   // т.к. запустив несколько notepad, работаем последовательно с ними
   // и выход из них такой же, т.е. нет одновременного доступа дл€
...
FUNCTION Tsb_Create( oBrw, aClrBC, aClrText )
   ...
   LOCAL aHead := { "єє", "Status", "File", "Size", "Date, time", "File", "Path" }
   ...
   WITH OBJECT oBrw
      ...
      :lDrawFooters  := .T.              // draw cellars
      :nClrLine      := RGB(43,149,168)  // line color between table cells
      ...
      // delete/add record - reserve
      //:UserKeys(VK_DELETE, {|ob| _wPost(30, ob, ob), .F. })
      //:UserKeys(VK_INSERT, {|ob| _wPost(40, ob, ob), .F. })

      // Keystroke analysis PgUp, PgDown
      //:bOnDraw := { || MsgInfo("key pressed: "+STR(nKey) ) }
...
FUNCTION myRunFiles(oWnd, nKy, xPar3, oBrw)
   ...
   ? "oWnd:Name =", cWnd, "oBrw:cAlias=", cAls

   nRecno := (cAls)->( Recno() )
   aFile  := { nRecno, ALLTRIM( (cAls)->F6 ) , ALLTRIM( (cAls)->F5 ), (cAls)->F4 }
   ? ; ? "aFile= " + HB_ValToExp(aFile)
   ? Repl("-",LINE_FILL)
   LogScreen()  // read the log-file and output to the form

   myThreadFile(aFile,oWnd)
   ? ProcNL(), "myThreadFile(aFile,oWnd)"
...
FUNCTION myDebugVar(oWnd,ky,xv)  // variable output
   ...
   IF cObj == "BUTTON_1"
      ...
      ? " ќбъект App.Cargo доступен всегда в любом месте программы"
   ...
   ELSE
      ...
      cMsg := "  oWnd:Cargo:"
      ? ProcNL(), "oWnd:Name =", oWnd:Name, oWnd,ky,xv
      ? " ќбъект окна доступен через переменную oWnd в функции или через App.Cargo:oWindow"
      ? " ¬ данном случае App.Cargo:oWindow это главное окно этой программы"
