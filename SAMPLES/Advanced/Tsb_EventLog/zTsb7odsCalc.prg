...
FUNCTION Brw7OleCalc( aTsb, aParam, aTitle, aFoot, aImage, hProgress, bExtern )
   ...
   WaitWindow( { cMsg, 'OO Calc OLE ...', App.Exename } , .T., 600, 16, NIL, YELLOW, LGREEN )
...
////   aProps[7]:Value := "MS Word 97"//Не работает
   ...
   // В этом варианте картинка рисуется из внешнего файла по запросу "Обновить все ссылки?"
   // При этом в сформированном файле она не сохраняется и при открытии файла заагружается заново
   //   If ! Empty( aImage )
            ...
            Do case
               ...
               case (rPicture != NIL).and.(aTipeChars[nColHead] =='N'.or.aTipeChars[nColHead] =='+'.or.aTipeChars[nColHead] =='^').and.Valtype(udata)='N'
                 ...
                 if nPoint == 0
                    ...
                    //Количесво после . плавующее   rPicture := Repl("#",nPoint-2) + '0.' + Repl("#",Len(rPicture)-nPoint)
