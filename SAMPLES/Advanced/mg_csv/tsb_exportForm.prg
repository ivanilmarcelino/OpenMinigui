...
FUNCTION Form_ExportFile(oWnd, cTitle, cMsg, cFileExp, cIco3, lSayCdPg)
   LOCAL cIcon, cFont, nFontSize, aBackColor, aBColorTxt, nI, aFont
   ...
   cFont      := aFont[1]                         // DejaVu Sans Mono
   nFontSize  := aFont[2]                   
   aFont      := GetFontParam( GetFontHandle( "ComSanMS" ) )
   cFontBtn   := aFont[1]                         // Comic Sans MS
   nFBtnSize  := aFont[2]                         
   aBtnFont   := { cFontBtn, nFBtnSize, .T. }     // фонт дл€ кнопок
   cFontTxt   := "Tahoma"
   aBackColor := ao:aBClr                         // цвет окна формы
   aBColorTxt := {255,255,240}
   cForm      := "Form_Export"
   cIcon      := ao:cDefAppIcon
   aIcon      := { cIco3, "iArrow64", "iCsv64" }
   ...
   DEFINE WINDOW &cForm AT nY, nX WIDTH nW HEIGHT nH ;
      ...
      cVal := IIF( ao:lRu, " »м€ файла дл€ экспорта ", " File name for export " )
      ...
      cVal := IIF( ao:lRu, " ѕуть дл€ экспорта файла ", " Path to export file " )
      ...
      /////////////////////// Button ////////////////////////////
      cVal  := IIF( ao:lRu, "”кажите путь дл€ экспорта", "Specify the path for export" )
      ...
      cVal := IIF( ao:lRu, " —оздание файла в кодировке ", " Creating a file in encoding " )
      ...
      cBtnCapt := IIF(ao:lRu, " одировка: ", "Charset: " ) + cCdPg
      cVal     := IIF( ao:lRu, "¬ыберите кодировку Ѕƒ", "Select DB encoding" )
      nYBtn    := nY
      nXBtn    := nX 
      nWBtn    := nWTxt2
      ...
      cVal  := IIF( ao:lRu, "«апрос на открытие файла после экспорта",;
      ...
      /////////////////////// Button ////////////////////////////
      cBtnCapt := IIF( ao:lRu, "ѕомощь", "Help" )
      nWBtn    := 300
      nHBtn    := 64 
      nYBtn    := nY
      nHIco    := nHBtn - nG/2

      cTxt  := IIF( ao:lRu, "Ёкспорт в файл", "Export to file" )
