...
/////////////////////////////////////////////////////////////////////////////////////////
FUNCTION AlertTsb7(cType,nIcoSize,aXDim,cTitle,aButton,oWin,o1Brw,bInitForm)
...
STATIC FUNCTION TsbPatam(cForm,aXDim,cBrw,aBColor,o1Brw,lIsDbf,aFontTsb)
   ...
   //oTsb:aField    := {"R_USR","R_PSW","R_ADM" ,"R_MGI"  ,"R_RWI","R_TXT", "R_ID" }
   //oTsb:aEdit     := { .F.  , .F.   , .F.     , .T.     , .T.   , .T.    }  // редактировать колонки
   //oTsb:aName     := oTsb:aField
   //oTsb:bGotFocus := {|ob| iif( IsObject(ob), _wPost(11, ob:cParentWnd, ob),) } - здесь не нужно
...
///////////////////////////////////////////////////////////////////////////////
STATIC FUNCTION myTsbListFont( oBrw )
...
STATIC FUNCTION MaxSize_TsbDbf(cAls, o1Brw, aFontTsb )
   ...
   // высота таблицы
   IF OrdCount() > 0
      nHTsb := OrdKeyCount() * aWH[3] + aWH[4] + aWH[5] + aWH[6] + aWH[7]
   ELSE
