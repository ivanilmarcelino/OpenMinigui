...
#xtranslate MiniGuiVersionChar()  => Substr( MiniGuiVersion(), At(".", MiniGuiVersion()) - 2, 5 )
#xtranslate MiniGuiVersionNumba() => Int( Val( MiniGuiVersionChar() ) * 10000 + Val( Right(MiniGuiVersionChar(), 2) ) )
////////////////////////////////////////////////////////////////////////
FUNCTION MG_Version( lNum )
   IF lNum == Nil      ; RETURN MiniGuiVersion()
   ELSEIF !Empty(lNum) ; RETURN MiniGuiVersionNumba()
   ENDIF
RETURN MiniGuiVersionChar()

*----------------------------------------------------------------------------*
FUNCTION SetsEnv(cFont, nSize, cFDlg, nFDlg, cLog, cIconDef, aBClrDlg)
*----------------------------------------------------------------------------*
   LOCAL o
...
FUNCTION MsgAbout(hIcon,nIcoSize)
   ...
   ELSE
      // Это только с версии 23.12 и выше
