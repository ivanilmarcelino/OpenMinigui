...
{
   HB_FIBITMAP **ptr = ( HB_FIBITMAP ** ) hb_gcAllocate( sizeof( HB_FIBITMAP * ), &s_gcFIBITMAPFuncs );
...
{
   FIMULTIBITMAP  **ptr = ( FIMULTIBITMAP ** ) hb_gcAllocate( sizeof( FIMULTIBITMAP * ), &s_gcFIMULTIBITMAPFuncs );
...
HB_FUNC( FI_GETMETADATACOUNT )
{
   FREE_IMAGE_MDMODEL   mdModel = ( FREE_IMAGE_MDMODEL ) hb_parni( 1 ); // Get the metadata model
   FIBITMAP             *dib = hb_FIBITMAP_par( 2 );                    // Get the bitmap handle
   unsigned int         count = FreeImage_GetMetadataCount( mdModel, dib );

   hb_retni( count );   // Return the metadata count
}
...
HB_FUNC( FI_GETCOMMENTS )
{
   FREE_IMAGE_MDMODEL   model = ( FREE_IMAGE_MDMODEL ) hb_parnl( 1 );
   FITAG                *tag;
   FIMETADATA           *mdhandle;
   const char           *value;

   mdhandle = FreeImage_FindFirstMetadata( model, hb_FIBITMAP_par( 2 ), &tag );

   if( mdhandle )
   {
      hb_reta( 2 );
      hb_storvc( ( char * ) FreeImage_GetTagKey( tag ), -1, 1 );

      do
      {
         // convert the tag value to a string
         value = FreeImage_TagToString( model, tag, NULL );
         hb_storvc( ( char * ) value, -1, 2 );
      }
...
HB_FUNC( FI_SETCOMMENTS )
{
   FREE_IMAGE_MDMODEL   model = ( FREE_IMAGE_MDMODEL ) hb_parnl( 1 );
   FIBITMAP             *dib = hb_FIBITMAP_par( 2 );
   char                 *value = ( char * ) hb_parc( 3 );
   DWORD                value_size = hb_parnl( 4 );
   FITAG                *tag;
   ...
   if( tag )
   {
      ...
      FreeImage_SetTagLength( tag, value_size );
      FreeImage_SetTagCount( tag, value_size );
      FreeImage_SetTagType( tag, FIDT_ASCII );
      FreeImage_SetTagValue( tag, value );

      // store the tag
      hb_fi_retl( FreeImage_SetMetadata( model, dib, FreeImage_GetTagKey( tag ), tag ) );
...
#if defined( HB_LEGACY_LEVEL4 )
HB_FUNC_TRANSLATE( FI_ROTATECLASSIC, FI_ROTATE )
#endif
