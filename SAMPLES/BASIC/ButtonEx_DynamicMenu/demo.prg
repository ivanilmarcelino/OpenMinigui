...
FUNCTION Main()
   ...
   aBtn := {} // 1.NameObj     2.Text                3.ExtFuncRun or CodeBlock                     4.Icons.5         6.Colors.7          8.HotKey     9.nFSize
   AADD( aBtn, { "Btn_01", "1. Selection menu (pg)", {|p1,p2,p3,p4| MyTest(1,p1,p2,p3,p4)} , "iSantax1", "iSantax2", aGrOver , aGrFill , { VK_F1, 49 }, 0 } )
   AADD( aBtn, { "Btn_02", "2. Selection menu (pg)", {|           | MyTest(2)            } , "iFolder1", "iFolder2", aGrOver , aGrFill , { VK_F2, 50 }, 0 } )
   AADD( aBtn, { "Btn_03", "3. Selection menu (pg)", "MyTest(3)"                           , "iHPx1"   , "iHPx2"   , aGrOver , aGrFill , { VK_F3, 51 }, 0 } )
   AADD( aBtn, { "Btn_04", "4. Selection menu (pg)", "MyTest(4)"                           , "iHMGx1"  , "iHMGx2"  , aGrOver , aGrFill , { VK_F4, 52 }, 0 } )
   AADD( aBtn, { "Btn_05", "Exit programm"         , "MyExit(99)"                          , "iExitx1" , "iExitx2" , aGrOverX, aGrFillX,   VK_ESCAPE  , 0 } )
       ...
       @ 50 + nG, 260 LABEL Label_1 WIDTH nW - 260 HEIGHT nUnchangPart - 50 - nG ;
         ...
         FOR nI := 1 TO Len(aBtn)
             IF ! CRLF $ aBtn[ nI ][2]
                ...
                ENDIF
                aBtn[ nI ][9] := Min( nS, aBtn[ nI ][9] )  // записать в массив меню размер фонта
             ENDIF
         NEXT
...
STATIC FUNCTION InfoFormObj()
   LOCAL nI, aBtn, cN, nBtnH, nIcoH, cText, cFont, nFSize, aFSize := {}
...
STATIC FUNCTION ResizeForm(nUnchangPart,nG)
   ...
   FOR nI := 1 TO  Len(aBtn)
      ...
      nImgSize := INT(nBtnH - 5*2)  // размер иконки
      //  IMAGESIZE nImgSize, nImgSize ;
      This.&(cN).ImageWidth := nImgSize
      This.&(cN).ImageHeight := nImgSize

      This.&(cN).Icon := LoadIconByName( aBtn[nI,4], nImgSize, nImgSize )
