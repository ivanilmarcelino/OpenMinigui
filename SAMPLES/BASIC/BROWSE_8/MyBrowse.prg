...
FUNCTION Main()
   ...
   IF ! Empty( aNewFiles )
      ...
      FOR i := 1 TO nCamp
         ...
         AAdd( aLong, Max( 120, Min( 180, aEst[ i, 3 ] * 14 ) ) )
      NEXT
...
FUNCTION CreaBrowse( cBase, aNomb, aLong, aJust, aFtype )
   ...
   hBrw := GetControlHandle( 'Browse_1', 'oWndBase' )
   nBrwOldH := GetWindowHeight( hBrw )
   nBrwNewH := ListView_CalculateSize( hBrw, iif( nAltoPantalla < 800, 31, 41 ) )

   SetProperty( 'oWndBase', 'Height', nHeight - nBrwOldH + nBrwNewH )
   SetProperty( 'oWndBase', 'Browse_1', 'Height', nBrwNewH )
...
PROCEDURE MyFind()
   ...
   LOCAL cExpress, cExpErr, nCurRec

   IF _SetGetGlobal( "Express" ) == NIL

      STATIC Express AS GLOBAL VALUE ''

   ENDIF

   cExpress := InputBox( "Enter a Search Expression :", "Find", _SetGetGlobal( "Express" ) )

   IF ! Empty( cExpress )

      ASSIGN GLOBAL Express := cExpress
      ...
      IF Empty( cExpErr ) .AND. .NOT. "U" $ Type( cExpress )
         ...
         IF ( Alias() )->( Eof() )
            ...
            MsgStop( "There is not the expression : " + cExpress, "Warning" )
         ENDIF
